...
- [ ] `make check` (debug build + fast tests), or **`make ci-check`** (adds release compile), or **`make release-verify`** to mirror the tag workflow (clean + coverage); confirm PR CI and/or nightly coverage as needed
